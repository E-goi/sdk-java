...
public class SendSms {
  ...
  * @throws IOException if the JSON Object is invalid with respect to SendSms
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!SendSms.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in SendSms is not found in the empty JSON string", SendSms.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!SendSms.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `SendSms` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : SendSms.openapiRequiredFields) {
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!SendSms.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'SendSms' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<SendSms> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(SendSms.class));

       return (TypeAdapter<T>) new TypeAdapter<SendSms>() {
           @Override
           public void write(JsonWriter out, SendSms value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public SendSms read(JsonReader in) throws IOException {
 ...
 /**
  * Create an instance of SendSms given an JSON string
  *
  * @param jsonString JSON string
  * @return An instance of SendSms
  * @throws IOException if the JSON string is invalid with respect to SendSms
  */
  public static SendSms fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, SendSms.class);
  }

 /**
  * Convert an instance of SendSms to an JSON string
  *
  * @return JSON string
  */
