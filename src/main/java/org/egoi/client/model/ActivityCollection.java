...
public class ActivityCollection {
  ...
  * @throws IOException if the JSON Object is invalid with respect to ActivityCollection
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!ActivityCollection.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in ActivityCollection is not found in the empty JSON string", ActivityCollection.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!ActivityCollection.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `ActivityCollection` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }
      if (jsonObj.get("items") != null && !jsonObj.get("items").isJsonNull()) {
        ...
        if (jsonArrayitems != null) {
          ...
          for (int i = 0; i < jsonArrayitems.size(); i++) {
            ContactActivity.validateJsonObject(jsonArrayitems.get(i).getAsJsonObject());
          };
        }
      }
  }
