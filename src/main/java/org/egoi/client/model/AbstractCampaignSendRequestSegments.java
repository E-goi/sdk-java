...
public class AbstractCampaignSendRequestSegments {
  ...
  public AbstractCampaignSendRequestSegments() {
  }
  ...
  * @throws IOException if the JSON Object is invalid with respect to AbstractCampaignSendRequestSegments
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!AbstractCampaignSendRequestSegments.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in AbstractCampaignSendRequestSegments is not found in the empty JSON string", AbstractCampaignSendRequestSegments.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!AbstractCampaignSendRequestSegments.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `AbstractCampaignSendRequestSegments` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }
      // ensure the optional json data is an array if present
      if (jsonObj.get("normal") != null && !jsonObj.get("normal").isJsonArray()) {
        throw new IllegalArgumentException(String.format("Expected the field `normal` to be an array in the JSON string but got `%s`", jsonObj.get("normal").toString()));
      }
      // ensure the optional json data is an array if present
      if (jsonObj.get("auto") != null && !jsonObj.get("auto").isJsonArray()) {
        throw new IllegalArgumentException(String.format("Expected the field `auto` to be an array in the JSON string but got `%s`", jsonObj.get("auto").toString()));
      }
      // ensure the optional json data is an array if present
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!AbstractCampaignSendRequestSegments.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'AbstractCampaignSendRequestSegments' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<AbstractCampaignSendRequestSegments> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(AbstractCampaignSendRequestSegments.class));

       return (TypeAdapter<T>) new TypeAdapter<AbstractCampaignSendRequestSegments>() {
           @Override
           public void write(JsonWriter out, AbstractCampaignSendRequestSegments value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public AbstractCampaignSendRequestSegments read(JsonReader in) throws IOException {
 ...
 /**
  * Create an instance of AbstractCampaignSendRequestSegments given an JSON string
  *
  * @param jsonString JSON string
  * @return An instance of AbstractCampaignSendRequestSegments
  * @throws IOException if the JSON string is invalid with respect to AbstractCampaignSendRequestSegments
  */
  public static AbstractCampaignSendRequestSegments fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, AbstractCampaignSendRequestSegments.class);
  }

 /**
  * Convert an instance of AbstractCampaignSendRequestSegments to an JSON string
  *
  * @return JSON string
  */
